    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if args['attack'] == "white":
            ...
            attack = CarliniL2(sess, model, batch_size=1, max_iterations=args['maxiter'], print_every=args['print_every'], 
                     ...
                     adam_beta1=args['adam_beta1'], adam_beta2=args['adam_beta2'], defense=args['defense'], noise=args['noise'], k_samps=args['k_samps'], mix_defense=args['mixture'])
        else:
            ...
            attack = BlackBoxL2(sess, model, batch_size=128, max_iterations=args['maxiter'], print_every=args['print_every'], 
                     ...
                     init_size=args['init_size'], use_importance=not args['uniform'], defense=args['defense'], noise=args['noise'], k_samps=args['k_samps'], mix_defense=args['mixture'])
        ...
        for i in range(all_true_ids.size):
            ...
            img_no += 1
            timestart = time.time()
            adv, const, q_counter = attack.attack_batch(inputs, targets)
            total_q_counter+=q_counter
            ...
            print("[QSTATS][L4], query_count, {}, success_rate, {:.3f}, distortion, {:.5f}".format(total_q_counter / float(img_no), total_success / float(img_no), l2_distortion))
            sys.stdout.flush()
        print("[QSTATS][L5], query_count, {}, success_rate, {:.3f}, distortion, {:.5f}".format(total_q_counter / float(img_no), total_success / float(img_no), l2_distortion))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-D", "--defense", action='store_true', help="activate the defense strategies")
    parser.add_argument("-M", "--mixture", action='store_true', help="activate the mixture defense strategies")
    parser.add_argument("-N", "--noise", type=float, default=0.1)
    parser.add_argument("-k", "--k_samps", type=int, default=1)
