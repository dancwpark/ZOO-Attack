...
class BlackBoxL2:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
                 ...
                 init_size = 32, use_importance = True, defense=DEFENSE, noise=NOISE,
                 k_samps=1, mix_defense=False):
        """
        ...
        self.MIX_DEFENSE = mix_defense
        self.K_SAMPS = k_samps
        if save_ckpts:
        ...
        if self.DEFENSE:
           ...
           # self.output = tf.nn.softmax(self.output) # delete
            ...
            for i in range(self.K_SAMPS):                
                noise = tf.squeeze(tf.map_fn(gmm_noise, self.output))
                def_noise = def_noise+noise
            self.output = self.output + (def_noise / self.K_SAMPS)
    ...
    def blackbox_optimizer(self, iteration):
        ...
        for i in range(self.batch_size):
            var[i * 2 + 1].reshape(-1)[indice[i]] += 0.01
            var[i * 2 + 2].reshape(-1)[indice[i]] -= 0.01
    ...
    def attack_batch(self, img, lab):
        ...
        return o_bestattack, o_best_const, query_counter
