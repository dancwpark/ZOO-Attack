...
class ImageNet:
  def __init__(self, num_imgs=500):
    from multiprocessing import Pool
    pool = Pool(8)
    file_list = sorted(os.listdir(os.environ['IMAGENET']))
    random.shuffle(file_list)
    r = pool.map(readimg, file_list[:num_imgs])
    # print(file_list[:num_imgs])
