...
do
    FILENAME="./results/task3/T_$dataset.out"
    echo $first_line > $FILENAME
    for noise in 0 0.0001 0.001 0.01 0.1 0.25 0.5 1 10 50
    do
        single_line=$(python test_all.py -d $dataset -a $attack -N $noise -D -b 9 -n $num_imgs --solver adam $untargeted | grep L5)
        #single_line=$(python sample_output.py | grep L4)
        echo $single_line
...
for noise in 0 0.0001 0.001 0.01 0.1 0.25 0.5 1 10 50
do
    single_line=$(python test_all.py -d $dataset -a $attack -N $noise -D -f 123 --solver adam -b 1 -c 10.0 --use_resize --reset_adam -m 3000 -n $num_imgs $untargeted | grep L5)
    #single_line=$(python sample_output.py | grep L4)
    echo $single_line
    A="$(cut -d',' -f3 <<< $single_line)"
    C="$(cut -d',' -f5 <<< $single_line)"
